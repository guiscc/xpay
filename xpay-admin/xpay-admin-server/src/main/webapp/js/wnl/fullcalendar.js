		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				...
				if(j==0||j==week.length-1){
					//判断工作日
					if(sunSa.indexOf(day[1]+","+day[2])==-1){
						jobDay=true;
					}
				}
				var dayHtml = '<font size="5" face="Arial Black">' + day[2];
				var info = null;
				var fastivalCheck = null;
				if (opts.month == day[1]) {
					info = currentCa[day[2] - 1];
					if (info.color) {
						fastivalCheck = true;
						var color = 'color="' + info.color + '"';
						dayHtml = '<font size="5" face="Arial Black" >' + day[2];
					}
					dayHtml += '</font>';
					if (opts.lunarDay) {
						dayHtml += "<br/>";
						if (opts.solarTerms && info.solarTerms) {
							dayHtml += '<font color="blue">' + info.solarTerms + '</font>';//节气
						} else {
							dayHtml += '<font size="2" style="font-size:9pt">' + cDay(info.lDay, info.lMonth) + '</font>';
						}
					}
				}
				if(null!=fastivalCheck||null!=jobDay){
					if(jobDay){
						var day = $('<td class="calendar-day fullcalendar-day calendar-other-month festival calendar-saturday"></td>').data('info', info).attr('abbr', day[0] + ',' + day[1] + ',' + day[2]).html(dayHtml).appendTo(tr);
					}else{
						var day = $('<td class="calendar-day fullcalendar-day calendar-other-month festival"></td>').data('info', info).attr('abbr', day[0] + ',' + day[1] + ',' + day[2]).html(dayHtml).appendTo(tr);
					}
				}else{
...
//		t.find('tr').find('td:eq(' + saIndex + ')').addClass('festival');
//		t.find('tr').find('td:eq(' + suIndex + ')').addClass('calendar-saturday');
