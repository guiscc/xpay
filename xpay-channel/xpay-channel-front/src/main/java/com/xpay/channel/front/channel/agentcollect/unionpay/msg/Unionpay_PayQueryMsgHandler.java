...
import com.xpay.channel.front.msg.impl.JsonChannelMsgHandlerImpl;
import com.xpay.channel.front.tongxin.impl.HttpFormChannelHandler;
...
public class Unionpay_PayQueryMsgHandler
    ...
    public ACQueryPayReqFrontDTO beforBuildMsg(ACQueryPayReqFrontDTO req, ChannelConfig channelConfig) throws BuildMsgException {
        ...
        String pwd = channelConfig.getPfxPwd();
        String sign = null;

        config.setBankURL(config.getPayQueryUrl());

        try {
//            sign = UnionpayUtil.sign(data, channelConfig.getCharset(), pfxPath, pwd);
//            data.put("signature", sign);
    ...
    @Override
    public ACQueryPayRepFrontDTO resolveMsg(ACQueryPayReqFrontDTO req, byte[] rtnMsg,
